...
// - GET /api/period-category-standings?periodId=1|P1|Period%201
...
async function fetchJsonPublic<T>(url: string): Promise<T> {
  const res = await fetch(url, {
    headers: { Accept: "application/json" },
    credentials: "omit", // critical: do NOT send cookies to MLB API
  });
...
/** ---------- NEW: Period Category Standings (server-computed) ---------- */
...
export type PeriodCategoryStandingRow = {
  ...
  value: number;
  rank: number; // 1..N
  points: number; // roto points, ties averaged
};
...
export type PeriodCategoryStandingsResponse = {
  // server may echo the original param ("P1") and also provide numeric periodNum
...
/**
 * NEW: Server-computed Period Category Standings
 * Examples:
 * - getPeriodCategoryStandings(1)
 * - getPeriodCategoryStandings("P1")
 * - getPeriodCategoryStandings("Period 1")
 */
export async function getPeriodCategoryStandings(
  periodId: string | number
): Promise<PeriodCategoryStandingsResponse> {
  ...
  const p = fetchJsonApi<PeriodCategoryStandingsResponse>(
    `${API_BASE}/period-category-standings?periodId=${encodeURIComponent(key)}`
  );
...
export async function getSeasonStandings(): Promise<SeasonStandingsApiResponse> {
  if (!_seasonStandingsCache) {
    _seasonStandingsCache = (async () => {
      const raw = await fetchJsonApi<any>(`${API_BASE}/season-standings`);

      // Shape A: already normalized
      if (raw && Array.isArray(raw.rows)) {
        const periodIds = Array.isArray(raw.periodIds)
          ? raw.periodIds.map((x: any) => Number(x)).filter(Number.isFinite)
          : [];
...
export async function getPeriodStandings(periodId: number): Promise<PeriodStandingsResponse> {
  ...
  try {
    ...
    const serverResp = await fetchJsonApi<any>(url);
